  ...
  return didError ? (
    ...
    >
      <div className="flex items-center justify-center w-full h-full">
        <Image src={ERROR_IMG_SRC} alt="Error loading image" width={88} height={88} style={{objectFit:'contain'}} data-original-url={safeSrc} />
      </div>
    </div>
  ) : (
    <Image src={safeSrc} alt={alt ?? ''} className={className} style={style} width={88} height={88} onError={handleError} />
  )
}
